...
import { getPageImageValidationError, PAGE_IMAGE_MAX_BYTES } from "@/service/pages/page-image";
...
describe("getPageImageValidationError", () => {
	it("rejects files larger than 3MB", () => {
		...
		expect(getPageImageValidationError(file)).toBe("File size must be 3MB or less.");
	});
