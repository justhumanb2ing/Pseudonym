...
describe("createPageImageUploader", () => {
	it("uploads to a stable path and appends a cache key", async () => {
		...
		const uploadPageImage = createPageImageUploader(Promise.resolve(supabase as never));
	...
	it("throws when upload fails", async () => {
		...
		await expect(uploadPageImage({ pageId: "page-1", userId: "user-1", file })).rejects.toThrow("upload failed");
	});
	...
	it("throws when the public url is missing", async () => {
		...
		await expect(uploadPageImage({ pageId: "page-1", userId: "user-1", file })).rejects.toThrow("Failed to resolve image URL.");
	});
