...
describe("PageDetailsEditor", () => {
	it("renders a dialog editor on desktop with length limits", async () => {
		...
		render(<PageDetailsEditor pageId="page-1" title="My Page" description="Hello" />);

		await user.click(screen.getByRole("button", { name: /My Page/ }));

		const titleInput = screen.getByLabelText("Title") as HTMLInputElement;
		const descriptionInput = screen.getByLabelText("Bio") as HTMLTextAreaElement;
		const intentInput = document.querySelector("input[name='intent']") as HTMLInputElement | null;
		...
		await waitFor(() => expect(screen.getByText(new RegExp(`^\\d+\\s*/\\s*${PAGE_TITLE_MAX_LENGTH}$`))).toBeInTheDocument());
	});
	...
	it("renders a drawer editor on smaller screens", async () => {
		...
		render(<PageDetailsEditor pageId="page-2" title="Mobile Page" description={null} />);
