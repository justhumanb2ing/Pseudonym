...
vi.mock("@/components/ui/cropper", () => ({
	Cropper: ({ children }: { children: React.ReactNode }) => <div data-testid="cropper">{children}</div>,
...
describe("ProfileImageUploader", () => {
	...
	it("shows a toast when the file exceeds 3MB", async () => {
		...
		render(<ProfileImageUploader pageId="page-1" userId="user-1" imageUrl={null} alt="Profile image" />);

		const input = screen.getByLabelText("Profile image upload");
		const file = new File([new Uint8Array(PAGE_IMAGE_MAX_BYTES + 1)], "big.png", { type: "image/png" });
	...
	it("does not show a cancel toast if upload is not in progress", async () => {
		...
		expect(toastManager.add).not.toHaveBeenCalledWith(expect.objectContaining({ title: "Upload canceled" }));
	});
	...
	it("shows a cancel toast when upload is in progress", async () => {
		...
		expect(toastManager.add).toHaveBeenCalledWith(expect.objectContaining({ title: "Upload canceled" }));
