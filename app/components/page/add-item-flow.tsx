...
export default function AddItemFlow({ pageId }: AddItemFlowProps) {
	...
	const content = (
		<div className="w-full py-4">
			<Tabs
				...
				className={cn("w-full gap-6", isDesktop ? "h-full flex-row" : "flex-col")}
			>
				<div className={cn("w-full", isDesktop ? "max-w-[220px]" : "px-0")}>
					<TabsList
						variant="default"
						className={cn(
							"scrollbar-hide w-full bg-background",
							isDesktop ? "flex-col items-stretch gap-1.5 p-1" : "flex-row gap-2 overflow-x-auto p-1",
						)}
					>
						{ITEM_TYPES.map((item) => (
							<TabsTrigger key={item.id} value={item.id} className={cn("min-h-10 font-normal", isDesktop ? "justify-start" : "shrink-0")}>
								{item.title}
							</TabsTrigger>
						))}
					</TabsList>
				</div>
				<div className={cn("h-full w-full", isDesktop ? "pr-6" : "px-5")}>
					...
					{ITEM_TYPES.filter((item) => item.id !== "link").map((item) => (
						<TabsContent key={item.id} value={item.id} className="flex min-h-[300px] flex-col">
							<div className="flex flex-col gap-3 rounded-xl px-4 py-6">
								<span className="font-semibold text-foreground text-sm">{`Add ${item.title.toLowerCase()}`}</span>
								<p className="text-muted-foreground text-xs">This flow is coming soon. Select another item to switch.</p>
							</div>
						</TabsContent>
					))}
				</div>
		...
		return (
			<Drawer open={open} onOpenChange={setOpen}>
				<DrawerTrigger asChild>
					<Button type="button" variant="brand" size="lg" onClick={() => setOpen(true)}>
						<IconPlus />
						Add
					</Button>
				</DrawerTrigger>
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			/>
			<DialogContent showCloseButton={false} className="min-h-[520px] rounded-2xl bg-background md:max-w-3xl">
