...
export function TextItemExpandedContent({ item }: { item: ProfileItem }) {
	...
	const formId = `text-item-edit-${item.id}`;
	const configData = item.config?.data;
	const defaultTitle = configData?.title ?? "";
	const updateError =
		updateFetcher.data?.intent === "text-update" && !updateFetcher.data?.success ? updateFetcher.data.formError : undefined;
	...
	return (
		<div className="flex h-full w-full flex-col justify-between gap-5">
			<updateFetcher.Form id={formId} method="post" className="flex flex-col gap-4">
				<input type="hidden" name="intent" value="text-update" />
				<input type="hidden" name="itemId" value={item.id} />
				<Field>
					<FieldContent>
						<div className="relative">
							<FieldLabel
								htmlFor={`text-item-title-${item.id}`}
							...
							<Textarea
								id={`text-item-title-${item.id}`}
								name="title"
								defaultValue={defaultTitle}
								autoComplete="off"
								required
								placeholder="Enter text..."
								className="max-h-40 overflow-y-auto rounded-lg border-0 px-3 pt-8"
							/>
						</div>
					</FieldContent>
				</Field>
