 ...
 */
export async function crawlLinkUrl(
  inputUrl: string,
  options: CrawlLinkOptions = {}
  ...
  const maxAttempts = Math.min(
    options.maxAttempts ?? RETRY_DELAYS_MS.length,
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      ...
      lastError =
        error instanceof Error ? error : new Error("Crawl failed.");
...
function resolveCrawlerBaseUrl() {
  const endpoint =
    import.meta.env.VITE_CRAWLER_SERVER_ENDPOINT ??
    process.env.VITE_CRAWLER_SERVER_ENDPOINT;
