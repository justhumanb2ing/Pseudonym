import { UMAMI_PROP_KEYS, type UmamiEventName, type UmamiPropKey } from "@/lib/umami-events";
...
export function resolveUmamiClientConfig(): UmamiClientConfig {
	...
	const environment = import.meta.env.VITE_APP_ENV ?? import.meta.env.MODE ?? "unknown";
 ...
 */
export function getUmamiEventAttributes(event: UmamiEventName, props?: UmamiEventProps): Record<string, string> {
	...
	for (const [key, value] of Object.entries(sanitizedProps)) {
		...
		attributes[`data-umami-event-${toAttributeKey(key)}`] = toAttributeValue(value);
	}
 ...
 */
export function trackUmamiEvent(event: UmamiEventName, props?: UmamiEventProps, options?: UmamiTrackOptions) {
 ...
 */
export function trackUmamiPageView(params: { url: string; title?: string; props?: UmamiEventProps; options?: UmamiTrackOptions }) {
