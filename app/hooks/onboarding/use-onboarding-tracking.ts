...
export function useOnboardingTracking() {
	...
	const trackSignupSuccess = useCallback((handle: string) => {
		trackUmamiEvent(
			UMAMI_EVENTS.auth.signup.success,
			{
				[UMAMI_PROP_KEYS.ctx.attemptId]: lastSubmitAttemptRef.current ?? signupAttemptIdRef.current,
	...
	const trackSignupError = useCallback((currentStep: Step, errorType: "server" | "validation") => {
		trackUmamiEvent(
